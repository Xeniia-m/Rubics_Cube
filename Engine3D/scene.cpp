...
bool Scene::comp_normals(int x, int y, int z, glm::vec4 wall_normal) {
	if (is_equal(z, (int)round(wall_normal.z * 10)) &&
		is_equal(y, (int)round(wall_normal.y * 10)) &&
		is_equal(x, (int)round(wall_normal.x * 10)))
		return true;
...
bool Scene::Picking(unsigned char data[4],float x,float y)
{
	...
	printf("picked: %f\n\n", picked_f);
	pickedShape = (int)round(picked_f)-1;
	if (pickedShape != -1) {
		glm::mat4 Normal = MakeTrans();
		...
		printf("normalx  : (%d, %d, %d)\n", (int)round(normal_x.x * 10.0), (int)round(normal_x.y * 10.0), (int)round(normal_x.z * 10.0));
		printf("normaly  : (%d, %d, %d)\n", (int)round(normal_y.x * 10.0), (int)round(normal_y.y * 10.0), (int)round(normal_y.z * 10.0));
		printf("normalz  : (%d, %d, %d)\n\n", (int)round(normal_z.x * 10.0), (int)round(normal_z.y * 10.0), (int)round(normal_z.z * 10.0));
		printf("normalx -: (%d, %d, %d)\n", (int)round(normal_x_.x * 10.0), (int)round(normal_x_.y * 10.0), (int)round(normal_x_.z * 10.0));
		printf("normaly -: (%d, %d, %d)\n", (int)round(normal_y_.x * 10.0), (int)round(normal_y_.y * 10.0), (int)round(normal_y_.z * 10.0));
		printf("normalz -: (%d, %d, %d)\n\n", (int)round(normal_z_.x * 10.0), (int)round(normal_z_.y * 10.0), (int)round(normal_z_.z * 10.0));

		glm::vec3 normal_pos = glm::vec3((data[1] / 255.0), (data[2] / 255.0), (data[3] / 255.0));
		printf("normalx_pos: (%d, %d, %d)\n", (int)round(normal_pos.x * 10), (int)round(normal_pos.y * 10.0), (int)round(normal_pos.z * 10.0));

		int x_normal = (int)round(normal_pos.x * 10);
		int y_normal = (int)round(normal_pos.y * 10);
		int z_normal = (int)round(normal_pos.z * 10);
